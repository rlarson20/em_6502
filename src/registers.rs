...
//Registers
//
//The 6502 has only a small number of registers compared to other processor of
//the same era. This makes it especially challenging to program as algorithms
//must make efficient use of both registers and memory.
//
//Program Counter
//
...
//The CPU does not detect if the stack is overflowed by excessive
//pushing or pulling operations and will most likely result
...
//The contents of the accumulator can be stored and retrieved
//either from memory or the stack.
//Most complex operations will need to use the accumulator for
//arithmetic and efficient optimisation of its use is a
//key feature of time critical routines.
//
//Index Register X
//
//The 8 bit index register is most commonly used to hold counters or offsets
//for accessing memory. The value of the X register can be loaded and
//saved in memory, compared with values held in memory or
//incremented and decremented.
//The X register has one special function.
...
//memory load, save and compare operations as wells as increments and decrements.
...
//control flags are held in a special status register.
//Each flag has a single bit within the register.
//Instructions exist to test the values of the various bits,
//to set or clear some of them and to push or pull the
...
//arithmetic, comparison and during logical shifts.
//It can be explicitly set using the 'Set Carry Flag'
...
//While this flag is set the processor will not respond to
//interrupts from devices until it is cleared by a
...
//ending up with a negative result: 64 + 64 => -128).
//It is determined by looking at the carry between bits 6 and 7
//and between bit 7 and the carry flag.
//
//Negative Flag
//
